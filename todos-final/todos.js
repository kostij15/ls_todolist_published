...
app.use(session({
  ...
  secret: config.SECRET,//"this is not very secure",
  store: new LokiStore({}),
}));
...
app.post("/lists/:todoListId/complete_all",
  ...
  catchError(async (req, res,) => {
    let todoListId = req.params.todoListId;
    let todoListCompleted = await res.locals.store.completeAllTodos(+todoListId);
    if (!todoListCompleted) throw new Error("Not found.");
...
app.post("/lists/:todoListId/destroy",
  ...
  catchError(async (req, res) => {
    let todoListId = +req.params.todoListId;
    let removed = await res.locals.store.deleteTodoList(todoListId);
  ...
  catchError(async (req, res) => {
    ...
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        let updated = await store.setTodoListTitle(+todoListId,
          todoListTitle);
